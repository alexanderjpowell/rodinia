...
function GICOV_CUDA(dev, host_grad_x, host_grad_y, GICOV_constants)
    ...
    @cuda (num_blocks, threads_per_block) GICOV_kernel(size(device_grad_y, 1),
            ...
            size(device_grad_y, 2), pointer(device_grad_x), GICOV_constants.c_cos_angle,
...
function dilate_kernel(img_dev_size1, img_dev_size2, img_dev_ptr, c_strel, dilated_out)
    ...
    el_center_i = fld1(size(c_strel,1),2)
    el_center_j = fld1(size(c_strel,2),2)
    ...
    i = mod1(thread_id, img_m)
    j = fld1(thread_id, img_m)
